...
def _merge_paths(scenario: str) -> tuple[str, str, str]:
...
def _run_merge(scenario: str, output_name: str) -> str:
    """Run merge for manual/visual verification in MuseScore."""
    base_path, head_path, user_path = _merge_paths(scenario)
...
def test_default_merge_visual():
    output_path = _run_merge("default", "merged.mscz")
    warnings.warn(
        f"Open in MuseScore and verify the default merge looks correct: {output_path}"
    )


def test_measure_added_merge_visual():
    output_path = _run_merge("measure-added", "merged.mscz")
    warnings.warn(
        f"Open in MuseScore and verify the measure-added merge looks correct: {output_path}"
    )


def test_merge_conflict_single_measure_raises():
    base_path, head_path, user_path = _merge_paths("merge-conflict-single-measure")
