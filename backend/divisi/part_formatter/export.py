...
def export_score_and_parts_ms4_storage_scoreparts(input_key, output_prefix, mscore_bin=None):
    """
    ...
    candidate_bins = ([mscore_bin] if mscore_bin else []) + \
                     [os.environ.get("MSCORE_BIN"), "mscore4", "mscore4portable", "mscore"]
    ...
    for b in candidate_bins:
        try:
            ...
            continue
    if not msbin:
        return {"status": "error", "details": "Could not locate MuseScore CLI (mscore4/mscore4portable)."}
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            with default_storage.open(input_key, "rb") as src, open(temp_input, "wb") as dst:
                dst.write(src.read())
        except Exception as e:
        ...
        try:
            proc = subprocess.run([msbin, "--score-parts-pdf", temp_input],
        ...
        for part in parts_list:
            try:
                if isinstance(part, dict):
                    name = part.get("name") or part.get("partName") or part.get("part") or "Part"
                    b64 = part.get("bin") or part.get("pdf") or part.get("pdfBin") or part.get("partsBin")
                ...
                safe_name = "".join(c for c in str(name) if c not in r'\/:*?"<>|').strip() or "Part"
        ...
        if not written:
            return {"status": "error", "details": "MuseScore ran but no PDFs were produced or decoded."}
