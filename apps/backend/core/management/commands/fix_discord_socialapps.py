...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        has_settings_config = (
            ...
            and "discord" in settings.SOCIALACCOUNT_PROVIDERS
            and "APP" in settings.SOCIALACCOUNT_PROVIDERS["discord"]
            and settings.SOCIALACCOUNT_PROVIDERS["discord"]["APP"].get("client_id")
        )
        ...
        if app_count == 0:
            self.stdout.write(
                self.style.SUCCESS(
                    "No Discord SocialApp entries found in database. Nothing to fix."
                )
            )
            return

        self.stdout.write(
            f"Found {app_count} Discord SocialApp entry/entries in database."
        )
        ...
        for app in discord_apps:
            ...
            if dry_run:
                site_info = (
                    f" (sites: {', '.join(str(s.id) for s in app.sites.all())})"
                    if app.sites.exists()
                    else " (no sites)"
                )
                self.stdout.write(
                    f"  [DRY RUN] Would delete SocialApp ID: {app.id}{site_info}"
                )
            else:
                site_info = (
                    f" (sites: {', '.join(str(s.id) for s in app.sites.all())})"
