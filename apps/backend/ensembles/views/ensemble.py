...
class EnsembleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action == "list":
            return (
                base_qs.annotate(arrangements_count=Count("arrangements", distinct=True))
                .prefetch_related(
                    Prefetch(
                        "userships",
        ...
        return base_qs.prefetch_related(
            ...
            Prefetch(
                "part_books",
                queryset=PartBook.objects.select_related("part_name").order_by("part_name__display_name", "-revision"),
            ),
        ).distinct()
    ...
    def upload_template(self, request, slug=None):
        ...
        serializer = UploadEnsembleTemplateSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
        return Response({}, status=201)

    @action(detail=True, methods=["get"])
    def download_template(self, request, slug=None):
        ensemble = self.get_object()
        if not self._has_access(ensemble, request.user):
            return Response(
                {"detail": "You do not have access to this ensemble."},
        ...
        template = EnsembleTemplate.get_latest_for_ensemble(ensemble)
        if not template:
            return Response({"detail": "Ensemble has no template set"}, status=404)

        file_handle = default_storage.open(template.file_key, "rb")
        return FileResponse(
            file_handle,
            as_attachment=True,
            filename="template.mscz",
    ...
    def rename_part_name(self, request, slug=None):
        ...
        serializer = EnsemblePartNameRenameSerializer(data=request.data, context={"ensemble": ensemble})
        serializer.is_valid(raise_exception=True)
    ...
    def merge_part_names(self, request, slug=None):
        ...
        serializer = EnsemblePartNameMergeSerializer(data=request.data, context={"ensemble": ensemble})
        serializer.is_valid(raise_exception=True)
