...
def test_export_respects_part_name_aliases(mock_render, arrangement_versions):
    ...
    assert (
        PartName.objects.filter(
            ensemble=v1.ensemble, display_name__iexact="Flute I"
        ).count()
        == 0
    )
...
def test_download_part_endpoint(arrangement_versions, client):
    ...
    part = PartAssetFactory(arrangement_version=v1, file_key=file_key, is_score=False)
...
def test_merge_persists_alias_for_future_uploads():
    ...
    v2 = ArrangementVersionFactory(
        arrangement__ensemble=ensemble
    ...
    assert (
        PartName.objects.filter(
            ensemble=ensemble, display_name__iexact="Flute I"
        ).count()
        == 0
    )
...
def test_merge_and_reupload_uses_alias(mock_render):
    ...
    with zipfile.ZipFile(zip_a2, "w") as z:
        ...
        z.writestr(
            "Flute I.pdf", b"%PDF-1.4 flute i part"
        )  # Same raw name as A1 so alias is used
    ...
    assert (
        PartName.objects.filter(ensemble=ensemble, display_name="Flute").count() == 1
